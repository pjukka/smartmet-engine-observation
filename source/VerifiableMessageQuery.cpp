...
std::string VerifiableMessageQuery::getSQLStatement() const
{
  try
  {
    ...
    if (m_returnOnlyLatest)
    {
      ...
      {
        std::cerr
            << "warning: SmartMet::Engine::Observation::VerifiableMessageQuery::getSQLStatement - "
      ...
      // Make union from all the select statements. Each of the statements are identical but the
      // station_id differs.
...
void VerifiableMessageQuery::setQueryParams(const VerifiableMessageQueryParams* qParams)
{
  try
  {
    ...
    if (selectNames->empty())
    {
      ...
      // exception.setExceptionCode(Obs_EngineException::INVALID_SQL_STATEMENT);
    ...
    if (icaoCodes->empty())
    {
      ...
      // exception.setExceptionCode(Obs_EngineException::MISSING_PARAMETER_VALUE);
      exception.addDetail(msg.str());
      throw exception;
    }

    if (m_returnOnlyLatest)
    {  // Station_id list is used in getSQLStatement method.
    ...
    // At first order by message_time and message_type. There might be corrections
