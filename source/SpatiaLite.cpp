    ...
    : itsMaxInsertSize(max_insert_size)
{
  try
  {
    ...
    // Default is fully synchronous (2), with WAL normal (1) is supposedly better, for best speed we
...
void SpatiaLite::createGroupMembersTable()
{
  try
  {
    ...
    itsSession << "CREATE TABLE IF NOT EXISTS group_members ("
                  ...
                  "CONSTRAINT fk_station_groups FOREIGN KEY (group_id) REFERENCES station_groups "
                  "(group_id), "
                  "CONSTRAINT fk_stations FOREIGN KEY (fmisid) REFERENCES stations (fmisid)"
                  "); "
                  "CREATE INDEX IF NOT EXISTS gm_sg_idx ON group_members (group_id,fmisid);";
    tr.commit();
  }
  catch (...)
  {
...
void SpatiaLite::createFlashDataTable()
{
  try
  {
    ...
    {
      itsSession
          << "SELECT AddGeometryColumn('flash_data', 'stroke_location', 4326, 'POINT', 'XY');";
    }
    ...
    itsSession << "SELECT spatial_index_enabled FROM geometry_columns "
                  "WHERE f_table_name='flash_data' AND f_geometry_column = 'stroke_location';",
...
void SpatiaLite::initSpatialMetaData()
{
  try
  {
    ...
    itsSession << "SELECT name FROM sqlite_master WHERE type='table' AND name = 'spatial_ref_sys';",
...
void SpatiaLite::createStationTable()
{
  try
  {
    ...
    itsSession << "CREATE TABLE IF NOT EXISTS stations("
                  ...
                  "SELECT AddGeometryColumn('stations', 'the_geom', 4326, 'POINT', 'XY');";
...
boost::posix_time::ptime SpatiaLite::getLatestObservationTime()
{
  try
  {
    ...
    {
      // If there is no cached observations in the database, use default value of 24 hours
                                                            ...
                                                            const std::string time_field)
{
  try
  {
    ...
    {
      // If there is no cached observations in the database, return not_a_date_time
  ...
  while (true)
  {
    try
    {
      const char *sqltemplate =
          ...
          "(location_id, fmisid, country_id, location_start, location_end, longitude, latitude, x, "
          "y, "
    ...
    while (pos1 < flashCacheData.size())
    {
      ...
      for (std::size_t i = pos1; i < pos2; ++i)
      {
        ...
        std::string sqltemplate =
            ...
            "peak_current, sensors, freedom_degree, ellipse_angle, ellipse_major, "
            "ellipse_minor, "
            "chi_square, rise_time, ptz_time, cloud_indicator, angle_indicator, "
...
void SpatiaLite::updateStationsAndGroups(SmartMet::Spine::Stations &stations)
{
  try
  {
  ...
  try
  {
    ...
    for (const SmartMet::Spine::Station &station : stations)
    {
      ...
      insertsql << "INSERT OR REPLACE INTO stations "
                << "(fmisid, geoid, wmo, lpnn, station_formal_name, station_start, station_end, "
                   "the_geom) "
...
void SpatiaLite::updateStationGroups(const SmartMet::Spine::Stations &stations)
{
  try
  {
    // Locking handled by updateStationsAndGroups
    ...
    for (const SmartMet::Spine::Station &station : stations)
    {
      ...
      if (groupCount == 0)
      {
        // Insert a group member. Ignore if insertion fail (perhaps group_id or fmisid is not found
...
// Manipulates vector to string list, e.g. ids[0] = 2094, id[1] = 2095 => idstring = "2094,2095"
...
SmartMet::Spine::Stations SpatiaLite::findStationsByWMO(
    const Settings &settings, const map<int, SmartMet::Spine::Station> &stationIndex)
{
  try
  {
    SmartMet::Spine::Stations stations;
    string wmos = getIdString(settings.wmos);
    string wmosql = ("SELECT fmisid FROM stations s WHERE wmo IN(" + wmos +
                     ") AND DATETIME('now') BETWEEN s.station_start AND s.station_end;");

    soci::rowset<int> rs = (itsSession.prepare << wmosql);

    for (const auto &fmisid : rs)
      stations.push_back(stationIndex.at(fmisid));
...
SmartMet::Spine::Stations SpatiaLite::findStationsByLPNN(
    const Settings &settings, const map<int, SmartMet::Spine::Station> &stationIndex)
{
  try
  {
    SmartMet::Spine::Stations stations;
    string lpnns = getIdString(settings.lpnns);
    string lpnnsql = ("SELECT fmisid FROM stations s WHERE lpnn IN(" + lpnns +
                      ") AND DATETIME('now') BETWEEN s.station_start AND s.station_end;");

    soci::rowset<int> rs = (itsSession.prepare << lpnnsql);
    ...
    const boost::posix_time::ptime &endtime)
{
  try
  {
    ...
    {  // Station selection from a station group or groups.
      distancesql +=
          "group_members gm "
  ...
  try
  {
    ...
    if (settings.latest)
    {
      query =
          ...
          "FROM weather_data_qc data JOIN locations loc ON (data.fmisid = loc.fmisid) "
          ...
          param +
          ") "
          "GROUP BY data.fmisid, data.parameter, data.sensor_no, loc.location_id, "
          "loc.location_end, "
          "loc.latitude, loc.longitude, loc.elevation "
    ...
    else
    {
      query =
          ...
          param +
          ") "
          "GROUP BY data.fmisid, data.obstime, data.parameter, data.sensor_no, loc.location_id, "
    ...
    while (st.fetch())
    {
      ...
      // Should resize back to original size guarantee space for next iteration (SOCI manual)
    ...
    const Fmi::TimeZones &timezones)
{
  try
  {
    ...
    std::string query =
        ...
        "FROM observation_data data JOIN locations loc ON (data.fmisid = loc.fmisid) "
        "WHERE data.fmisid IN (" +
        qstations +
        ") "
        ...
        "GROUP BY data.fmisid, data.data_time, data.measurand_id, loc.location_id, "
        "loc.location_end, "
        "loc.latitude, loc.longitude, loc.elevation "
        "ORDER BY fmisid ASC, obstime ASC;";
        ...
        for (const SmartMet::Spine::Station &s : stations)
        {
          if (settings.latest)
          {
            ...
            for (const auto &special : specialPositions)
            {
              ...
              {
                // Feels like - deduction. This ignores radiation, since it is measured using
                // dedicated stations
                // dedicated stations
    ...
    const Settings &settings, const ParameterMap &parameterMap, const Fmi::TimeZones &timezones)
{
  try
  {
    ...
    std::string query;
    query =
        "SELECT DATETIME(stroke_time) AS stroke_time, stroke_time_fraction, flash_id, "
  ...
  try
  {
    ...
    else if (paramname == "wmo")
    {
      ...
      timeSeriesColumns->at(pos)
          .push_back(ts::TimedValue(obstime, station.wmo > 0 ? station.wmo : missing));
    }
    else if (paramname == "lpnn")
    {
      const ts::Value missing = ts::None();
      timeSeriesColumns->at(pos)
          .push_back(ts::TimedValue(obstime, station.lpnn > 0 ? station.lpnn : missing));
    }
    else if (paramname == "rwsid")
    {
      const ts::Value missing = ts::None();
      timeSeriesColumns->at(pos)
          .push_back(ts::TimedValue(obstime, station.rwsid > 0 ? station.rwsid : missing));
    }
    ...
    {
      std::string msg =
          "SpatiaLite::addSpecialParameterToTimeSeries : Unsupported special parameter '" +
          paramname + "'";
    ...
    const boost::posix_time::ptime &endtime)
{
  try
  {
    ...
    if (!stationgroup_codes.empty())
    {
      std::string query =
          "SELECT geoid, fmisid FROM stations s WHERE fmisid IN("
          "SELECT fmisid FROM "
          "group_members gm "
          "JOIN station_groups sg ON gm.group_id = sg.group_id ";
      ...
      auto it = stationgroup_codes.begin();
      query += "WHERE (sg.group_code = '" + *it + "' ";
      for (it++; it != stationgroup_codes.end(); it++)
      {
        query += "OR sg.group_code='" + *it + "' ";
      }
      query += ")) ";
      query +=
          "AND (:starttime BETWEEN s.station_start AND s.station_end OR "
      ...
      SociRowPtr rs;
      rs.reset(new SociRow(
          (itsSession.prepare << query, soci::use(to_tm(starttime)), soci::use(to_tm(endtime)))));
      ...
      for (const auto &row : *rs)
      {
        ...
        try
        {
          ...
          station = stationIndex[station_id];
          station.geoid = geoid;
        }
        catch (const std::bad_cast &e)
        {
...
SmartMet::Spine::Stations SpatiaLite::findStationsInsideArea(
    const Settings &settings,
    const std::string &areaWkt,
    map<int, SmartMet::Spine::Station> &stationIndex)
{
  try
  {
    SmartMet::Spine::Stations stations;
...
SmartMet::Spine::Stations SpatiaLite::findStationsInsideBox(
    const Settings &settings, const map<int, SmartMet::Spine::Station> &stationIndex)
{
  try
  {
    ...
    for (const auto &row : *rs)
    {
      ...
      try
      {
        int geoid = row.get<int>(0);
        int station_id = row.get<int>(1);
        station = stationIndex.at(station_id);
        station.geoid = geoid;
      }
      catch (const std::bad_cast &e)
      {
                             ...
                             const std::set<std::string> &stationgroup_codes)
{
  try
  {
    ...
    // Checking the default value of station_id and then data do the data population.
    if (fmisid)
    {
      if (missingStationId)
                                      ...
                                      const SmartMet::Spine::TaggedLocationList &locations)
{
  try
  {
    ...
    std::string sqltemplate =
        "SELECT "
        "IFNULL(SUM(CASE WHEN flash.multiplicity > 0 THEN 1 ELSE 0 END), 0) AS flashcount, "
        "IFNULL(SUM(CASE WHEN flash.multiplicity = 0 THEN 1 ELSE 0 END), 0) AS strokecount, "
        "IFNULL(SUM(CASE WHEN flash.cloud_indicator = 1 THEN 1 ELSE 0 END), 0) AS iccount "
        " FROM flash_data flash "
